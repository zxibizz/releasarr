...
import { connectionTestPayload, type ServiceIntegration } from '@/features/settings/serviceFields';
import type { SettingFieldInfo, SettingsSection } from '@/types';

interface ServiceSettingsModalProps {
  integration: ServiceIntegration;
  /** Every field this service owns, across sections. */
...
/**
 * Edits one service in isolation. Its fields are spread across two sections
 * server-side, so the save fans out; the draft is the only source for a
...
export function ServiceSettingsModal({
  ...
  const credentials = fields.filter((field) => field.section === 'services');
  const tuning = fields.filter((field) => field.section !== 'services');
  ...
  return (
    <ResponsiveModal opened onClose={onClose} title={t(`settings.services.${integration}`)}>
      <Stack gap="md">
