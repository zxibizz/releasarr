...
CREATE_RELEASE_RESPONSES = error_responses(
    {
        status.HTTP_400_BAD_REQUEST: "Malformed request payload.",
        status.HTTP_409_CONFLICT: "A release with the same identifier already exists.",
        status.HTTP_422_UNPROCESSABLE_CONTENT: "Validation failed for the provided fields.",
        status.HTTP_500_INTERNAL_SERVER_ERROR: _SERVER_ERROR,
    }
)

GET_RELEASE_RESPONSES = error_responses(
    {
...
DELETE_RELEASE_RESPONSES = error_responses(
    {
        status.HTTP_404_NOT_FOUND: "Release not found.",
        status.HTTP_500_INTERNAL_SERVER_ERROR: _SERVER_ERROR,
    }
...
PAUSE_RELEASE_RESPONSES = error_responses(
    {
...
RESUME_RELEASE_RESPONSES = error_responses(
    {
...
UPDATE_MAPPINGS_RESPONSES = error_responses(
    {
        status.HTTP_400_BAD_REQUEST: "Malformed request payload.",
...
REQUEST_RELEASES_RESPONSES = error_responses(
    {
        status.HTTP_400_BAD_REQUEST: "Invalid pagination or filter parameters.",
...
QUEUE_DOWNLOAD_RESPONSES = error_responses(
    {
...
def _decode_id(encoded_id: str) -> str:
    """Decode a URL-encoded Base64 release ID, falling back to the raw value."""
...
def _search_to_response(dto: ReleaseSearchResponseDTO) -> ReleaseSearchResponse:
    return ReleaseSearchResponse.model_validate(dto)
