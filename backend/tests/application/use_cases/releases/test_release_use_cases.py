...
async def test_pause_release_returns_async_operation() -> None:
    ...
    # The lifecycle service must be driven by the torrent info hash, not the DB id.
    assert lifecycle_service.pause_calls == ["hash-rel-1"]


@pytest.mark.asyncio
async def test_resume_release_uses_info_hash() -> None:
    release = make_release_record("rel-1")
    repository = FakeReleaseRepository({release.id: release})
    lifecycle_service = FakeLifecycleService()
    ...
    assert result.operation == "resume_release"
    assert lifecycle_service.resume_calls == ["hash-rel-1"]
